       ...
       # 段階別の追加報酬
        ...
        for p in players:
            ...
            if is_first_half:
                ...
                # ✅ 写入数据库字段
                p.final_points = first_half_points
                # 打印日志
                ...
                # ✅ 计算抽奖次数（每100点 = 1次）
                second_half_draws = math.floor(second_half_points / 100)
                p.participant.vars['second_half_draws'] = second_half_draws

                # ✅ 生成随机号码
                if second_half_draws > 0:
                    second_numbers = get_unique_numbers(second_half_draws)
                else:
                    second_numbers = []
                p.participant.vars['second_half_numbers'] = second_numbers

                # ✅ 立即保存抽奖号码到数据库
                p.final_points = second_half_points  # 确保最终点数也保存
                p.lottery_numbers_json = json.dumps(second_numbers)   # 保存抽奖号码到数据库
        ...
        if self.round_number == 5 and self.id_in_subsession == 1:
            ...
            print(Fore.CYAN + f"[{timestamp}] 🌏 【{group_label}】前半（差距较小的世界）总结" + Style.RESET_ALL)
            ...
            print("\n" + Fore.WHITE + "=" * 90 + Style.RESET_ALL)
            # 世界打印标题
            print(Fore.MAGENTA + f"[{timestamp}] 🌍 【{group_label}】后半（差距巨大的世界）总结" + Style.RESET_ALL)
            print(Fore.WHITE + "=" * 90 + Style.RESET_ALL)
            ...
            for p in players:
                ...
                draws = p.participant.vars.get("second_half_draws", 0)
                nums = p.participant.vars.get("second_half_numbers", [])
                print(f"  🎟️ 抽奖次数: {draws} 次 | 抽奖号码: {Fore.CYAN}{nums if nums else '（无）'}{Style.RESET_ALL}")
...
class QuizCheck(Page):
    template_name = 'exam_CN/QuizCheck.html'
    form_model = 'player'
...
class EffortDecision(Page):
    ...
    def vars_for_template(player: Player):
        if player.round_number in [1, 6]:# 🚫 第1轮 & 第6轮：不继承前一轮
            # 对于第1轮和第6轮，使用participant.vars中的points（对于第6轮，我们在IntermissionPage中重置为0）加上本轮发放的140
            base_points = player.participant.vars.get("points", C.INITIAL_POINTS)
            display_points = base_points + C.ROUND_ALLOWANCE
            prev_round = None
            previous_round_rank_val = None#(如果存在上一轮 (prev_round) 并且它有属性 round_rank，就取出上一轮的名次；否则保持为 None)
            total_hours = 0  # 第1轮和第6轮是新的开始，累计时间为0
            previous_points = 0  # 第1轮和第6轮没有上一轮，所以上一轮点数为0
...
# コスト表
            ...
            a_linear=C.A_LINEAR,
            cost_formula_text="第1時間=10ポイント。その後は限界コストが毎時間+2ずつ増えていきます",
...
class IntermissionPage(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        # 自分のデータをリセット
        #player.points = C.ROUND_ALLOWANCE  # 後半の初期支給ポイント
        #player.total_hours = 0             # 累積学習時間リセット
        #player.effort = None               # 努力値クリア
        #player.round_rank = 0
        #player.winner = False
        #player.final_points = 0
...
class LotteryDraw(Page):
    ...
    def vars_for_template(player: Player):
        # ✅ 修改为显示【后半世界】抽奖结果
        final_points = player.participant.vars.get("second_half_points", 0)
        lottery_draws = player.participant.vars.get("second_half_draws", 0)
        lottery_numbers = player.participant.vars.get("second_half_numbers", [])
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        final_points = player.participant.vars.get("second_half_points", 0)
        lottery_numbers = player.participant.vars.get("second_half_numbers", [])
        player.final_points = final_points
        player.lottery_numbers_json = json.dumps(lottery_numbers)
        
        # 数据已经在 determine_final_winner 中保存，这里只需标记
        player.did_draw = True
    
        print(f"[✅ Saved] Player {player.id_in_group} 抽奖结果已保存进数据库。")
